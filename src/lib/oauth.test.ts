...
describe('establishOAuthSession', () => {
  ...
  it('links, refreshes, then persists for a new identity (actor_id empty)', async () => {
    ...
    mockFetch
      ...
      .mockResolvedValueOnce(accessibleCustomer()) // assertCustomerAccess
  ...
  it('account-link is authorized with the pre-link token and the publishable key', async () => {
    ...
    mockFetch
      ...
      .mockResolvedValueOnce(accessibleCustomer())
  ...
  it('does not persist a session for a held OAuth customer', async () => {
    const token = fakeJwt({
      actor_id: 'cust_held',
      user_metadata: { email: 'held@example.com' },
    })
    mockFetch
      .mockResolvedValueOnce(okJson({ token }))
