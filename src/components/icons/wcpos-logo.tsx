import type { SVGProps } from 'react'

/**
 * WCPOS logo mark — the shop-awning speech bubble.
 *
 * Colours are hardcoded brand values (not currentColor) so the mark keeps
 * its identity in both themes. Defaults to aria-hidden because it always
 * sits next to a visible "WCPOS" wordmark; spread props to override.
 *
 * The same path data lives in src/app/icon.svg (the Next.js favicon file
 * convention needs a real file on disk) — keep the two in sync.
 */
export function WcposLogo(props: SVGProps<SVGSVGElement>) {
  return (
    <svg
      xmlns="http://www.w3.org/2000/svg"
      viewBox="0 0 1260 1260"
      aria-hidden="true"
      {...props}
    >
      <path
        d="M0,90 Q0,0 90,0 L1170,0 Q1260,0 1260,90 L1260,900 Q1260,990 1170,990 L360,990 L0,1260 Z"
        fill="#323A46"
      />
      <path
        fill="#CD2C24"
        d="M0,90 Q0,0 90,0 L180,0 L180,270 A90,90 0 1 1 0,270 Z
          M360,0 H540 V270 A90,90 0 1 1 360,270 Z
          M720,0 H900 V270 A90,90 0 1 1 720,270 Z
          M1080,0 H1170 Q1260,0 1260,90 L1260,270 A90,90 0 1 1 1080,270 Z"
      />
      <path
        fill="#F5E5C0"
        d="M180,0 H360 V270 A90,90 0 1 1 180,270 Z
          M540,0 H720 V270 A90,90 0 1 1 540,270 Z
          M900,0 H1080 V270 A90,90 0 1 1 900,270 Z"
      />
      <g fill="none" stroke="#FFF" strokeLinecap="round" strokeWidth={55}>
        <line x1={130} y1={923.5} x2={130} y2={532} />
        <circle cx={260.5} cy={662.5} r={130.5} />
        <circle cx={630} cy={662} r={131} />
        <path d="m1131,565.8c-21.75,-13.05 -65.25,-34.8 -130.5,-34.8q-130.5,0 -130.5,65.25t130.5,65.25t130.5,65.25t-130.5,65.25c-65.25,0 -108.75,-21.75 -130.5,-34.8" />
      </g>
    </svg>
  )
}
